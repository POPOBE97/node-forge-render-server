...
fn mc_math_closure_27751c37_(uv: vec2<f32>, uv_1: vec2<f32>) -> vec4<f32> {
    var uv_2: vec2<f32>;
    var uv_3: vec2<f32>;
...
fn sample_pass_texture(uv_in: vec2f) -> vec4f {
    return textureSample(pass_tex_PassTexture_58, pass_samp_PassTexture_58, uv_in);
}
...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    var math_closure_out_409dc96a: vec2f;
    {
        let xy = in.local_px.xy;
        let size = in.geo_size_px;
        var output: vec2f;
        output = mc_math_closure(in.uv, xy, size);
        math_closure_out_409dc96a = output;
    }
    var math_closure_out: vec4f;
    {
        let uv = math_closure_out_409dc96a;
        var output: vec4f;
        output = mc_math_closure_27751c37_(in.uv, uv);
