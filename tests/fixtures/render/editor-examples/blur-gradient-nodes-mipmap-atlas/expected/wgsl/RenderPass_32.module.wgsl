...
@group(1) @binding(0)
var pass_tex_PassTexture_33: texture_2d<f32>;

@group(1) @binding(1)
var pass_samp_PassTexture_33: sampler;
...
fn mc_math_closure_f6b31d5e(uv: vec2<f32>, uv_1: vec2<f32>) -> vec4<f32> {
    var uv_2: vec2<f32>;
    var uv_3: vec2<f32>;
...
fn sample_pass_texture(uv_in: vec2f) -> vec4f {
    return textureSample(pass_tex_PassTexture_33, pass_samp_PassTexture_33, uv_in);
}
...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    var math_closure_out_b601cdd6: vec2f;
    {
        let xy = in.local_px.xy;
        let size = in.geo_size_px;
        var output: vec2f;
        output = mc_math_closure(in.uv, xy, size);
        math_closure_out_b601cdd6 = output;
    }
    var math_closure_out: vec4f;
    {
        let uv = math_closure_out_b601cdd6;
        var output: vec4f;
        output = mc_math_closure_f6b31d5e(in.uv, uv);
