...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    // ImageTexture ImageTexture_24 aspect-correct uv
    let image_texture_uv = aspect_correct_uv_fill(
        (in.uv),
        vec2f(textureDimensions(img_tex_ImageTexture_24)),
        in.geo_size_px,
    );
    // ImageTexture ImageTexture_24.color
    let image_texture_sample = textureSample(img_tex_ImageTexture_24, img_samp_ImageTexture_24, image_texture_uv);
    // Final composite
