...
fn mc_math_closure_6c3850f2_(uv: vec2<f32>, uv_1: vec2<f32>) -> vec4<f32> {
    var uv_2: vec2<f32>;
    var uv_3: vec2<f32>;
...
fn sample_pass_texture(uv_in: vec2f) -> vec4f {
    return textureSample(pass_tex_PassTexture_62, pass_samp_PassTexture_62, uv_in);
}
 ...
 let rect_size_px_base = (graph_inputs.node_Vector2Input_83_e8df53bd).xy;
 let rect_center_px = (graph_inputs.node_Vector2Input_84_cdeb53bd).xy;
...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    var math_closure_out_b4a0c9a1: vec2f;
    {
        let xy = in.local_px.xy;
        let size = in.geo_size_px;
        var output: vec2f;
        output = mc_math_closure(in.uv, xy, size);
        math_closure_out_b4a0c9a1 = output;
    }
    var math_closure_out: vec4f;
    {
        let uv = math_closure_out_b4a0c9a1;
        var output: vec4f;
        output = mc_math_closure_6c3850f2_(in.uv, uv);
