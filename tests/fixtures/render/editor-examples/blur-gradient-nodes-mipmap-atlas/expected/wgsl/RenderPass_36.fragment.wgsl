...
@group(1) @binding(0)
var pass_tex_PassTexture_37: texture_2d<f32>;

@group(1) @binding(1)
var pass_samp_PassTexture_37: sampler;
...
fn mc_math_closure_d21bfdb4_(uv: vec2<f32>, uv_1: vec2<f32>) -> vec4<f32> {
    var uv_2: vec2<f32>;
    var uv_3: vec2<f32>;
...
fn sample_pass_texture(uv_in: vec2f) -> vec4f {
    return textureSample(pass_tex_PassTexture_37, pass_samp_PassTexture_37, uv_in);
}


@fragment
fn fs_main(in: VSOut) -> @location(0) vec4f {
    var math_closure_out_6a2fbe80: vec2f;
    {
        let xy = in.local_px.xy;
        let size = in.geo_size_px;
        var output: vec2f;
        output = mc_math_closure(in.uv, xy, size);
        math_closure_out_6a2fbe80 = output;
    }
    var math_closure_out: vec4f;
    {
        let uv = math_closure_out_6a2fbe80;
        var output: vec4f;
        output = mc_math_closure_d21bfdb4_(in.uv, uv);
