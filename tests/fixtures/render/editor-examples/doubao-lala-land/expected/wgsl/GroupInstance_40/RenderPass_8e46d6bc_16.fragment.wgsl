...
@group(1) @binding(0)
var pass_tex_GroupInstance_40_GuassianBlurPass_c0928d59_36: texture_2d<f32>;

@group(1) @binding(1)
var pass_samp_GroupInstance_40_GuassianBlurPass_c0928d59_36: sampler;
...
fn shader_material_GroupInstance_40_ShaderMaterial_faaaafde_35(
    in: ShaderMaterialInput,
    texture: texture_2d<f32>,
    texture_sampler: sampler,
) -> vec4f {
    let texture_size = vec2f(textureDimensions(texture));
    let source_origin = (texture_size - in.geometry_size) * 0.5;
    let source_px = source_origin + in.local_position.xy;
    let content = sample_texture_local_uv(
        texture,
        texture_sampler,
        source_px / texture_size,
    );
...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    // Shader Material GroupInstance_40/ShaderMaterial_faaaafde_35.material
    let shader_material_material = shader_material_GroupInstance_40_ShaderMaterial_faaaafde_35(
        ShaderMaterialInput(vec2f(in.uv.x, 1.0 - in.uv.y), in.frag_coord_gl, in.local_px, in.geo_size_px, params.target_size, params.time),
        pass_tex_GroupInstance_40_GuassianBlurPass_c0928d59_36,
        pass_samp_GroupInstance_40_GuassianBlurPass_c0928d59_36,
    );
    // Final composite
