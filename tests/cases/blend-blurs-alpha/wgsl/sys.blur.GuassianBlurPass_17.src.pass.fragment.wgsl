...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    // ImageTexture node_15 aspect-correct uv
    let image_texture_uv = aspect_correct_uv_fill((in.uv), vec2f(textureDimensions(img_tex_node_15)), in.geo_size_px);
    // ImageTexture node_15.color
    let image_texture_sample = textureSample(img_tex_node_15, img_samp_node_15, image_texture_uv);
    // Final composite
