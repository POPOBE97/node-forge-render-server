...
fn fs_main(in: VSOut) -> @location(0) vec4f {
    ...
    {
        let input1 = in.geo_size_px;
        var output: vec3f;
        output = mc_MathClosure_45_(in.uv, input1);
 ...
 var glass_out_GlassMaterial_40: vec4f;
 {
    ...
    let radius_px = (mc_MathClosure_45_out).z;
    ...
    let light_width = f32(30);
    let light_edge_pow = f32(2);
    ...
    let display_px = (screen_px - center_px) * f32(1) + center_px;
    let incident = normalize(vec3f(0.0, 0.0, -1.0));
    let refractive_index = mix(f32(1.5), f32(1.5) + 0.2, f32(1));
    ...
    let refraction = glass_texture_map(pass_tex_GuassianBlurPass_42, pass_samp_GuassianBlurPass_42, refract_uv, true, 0, vec2f(0, 1), pass_tex_GuassianBlurPass_42, pass_samp_GuassianBlurPass_42);
    ...
    let reflection = glass_texture_map(pass_tex_GuassianBlurPass_42, pass_samp_GuassianBlurPass_42, reflect_uv, true, 0, vec2f(0, 1), pass_tex_GuassianBlurPass_42, pass_samp_GuassianBlurPass_42);
    ...
    glass_color_ratio = mix(glass_color_ratio, vec4f(1, 1, 1, 0).rgb, vec4f(1, 1, 1, 0).a);
    glass_mat = vec4f(mix(glass_mat.rgb, glass_color_ratio, 0 * color_ratio), glass_mat.a);

    // Inner bottom gradient.
    glass_mat = vec4f(
        glass_mat.rgb + vec3f(pow(smoothstep(1.0, 0.0, uv.y), 2.0) * 0),
        glass_mat.a,
    );

    // Directional lighting.
    let light_strength = mix(f32(1), f32(1) + 1.0, f32(0));
    let light_strength_opp = f32(0);
    let l1 = glass_calculate_lighting(light_normal, vec3f(0.5, -0.699999988, 0.5), light_strength, 0.5);
    let l2 = glass_calculate_lighting(light_normal, vec3f(0.5, -0.699999988, 0.5) * vec3f(-1.0, -1.0, 1.0), light_strength_opp, 0.5);
    ...
    // Apply power curve.
    glass_mat = pow(glass_mat, vec4f(1));

    // Alpha.
    glass_mat = vec4f(glass_mat.rgb, glass_mat.a * final_alpha * 1);

    // Composite over background.
    // Use premultiplied alpha output.
    let out_rgb = glass_mat.rgb;
    let out_a = glass_mat.a;
    glass_out_GlassMaterial_40 = vec4f(out_rgb, out_a);
}

    return glass_out_GlassMaterial_40;
}
