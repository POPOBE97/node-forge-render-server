...
struct GraphInputs {
    // Node: Vector2Input_81
    node_Vector2Input_81_4ee353bd: vec4f,
    // Node: Vector2Input_82
    node_Vector2Input_82_9be153bd: vec4f,
};
...
fn mc_GroupInstance_60_MathClosure_30_(uv: vec2<f32>, xy: vec2<f32>, size: vec2<f32>) -> vec2<f32> {
 ...
 let rect_size_px_base = (graph_inputs.node_Vector2Input_81_4ee353bd).xy;
 let rect_center_px = (graph_inputs.node_Vector2Input_82_9be153bd).xy;
...
fn fs_main(in: VSOut) -> @location(0) vec4f {
        var mc_GroupInstance_60_MathClosure_30_out: vec2f;
    {
        let xy = in.local_px.xy;
        let size = in.geo_size_px;
        var output: vec2f;
        output = mc_GroupInstance_60_MathClosure_30_(in.uv, xy, size);
        mc_GroupInstance_60_MathClosure_30_out = output;
    }
    let _frag_out = textureSample(pass_tex_Downsample_16, pass_samp_Downsample_16, vec2f((mc_GroupInstance_60_MathClosure_30_out).x, 1.0 - (mc_GroupInstance_60_MathClosure_30_out).y));
