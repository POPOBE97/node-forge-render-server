use egui::Vec2;
use objc2_app_kit::{NSView, NSWindow, NSWindowButton, NSWindowCollectionBehavior};
...
pub(crate) fn apply_auxiliary_viewport_window_behavior(window_handle: &RawWindowHandle) -> bool {
    let RawWindowHandle::AppKit(appkit_handle) = window_handle else {
        return false;
    };

    let Some(ns_view) = ns_view_from_handle(appkit_handle) else {
        return false;
    };
