...
impl<'app> WgpuWinitApp<'app> {
    ...
    fn initialized_all_windows(&mut self, event_loop: &ActiveEventLoop) {
        ...
        for viewport in viewports.values_mut() {
            let is_child_viewport = viewport.ids.parent != viewport.ids.this;
    ...
    fn recreate_window(&self, event_loop: &ActiveEventLoop, running: &WgpuWinitRunning<'app>) {
        ...
        .initialize_window(event_loop, egui_ctx, false, viewport_from_window, painter);
    }
    ...
    fn initialize_window(
        ...
        match event_loop.create_window(window_attributes) {
            Ok(window) => {
                ...
                #[cfg(target_os = "macos")]
                if is_child_viewport {
                    if let Ok(handle) = window.window_handle() {
                        let raw_window_handle = handle.as_raw();
                        super::macos::apply_auxiliary_viewport_window_behavior(&raw_window_handle);
                    }
                }
...
fn render_immediate_viewport(
    ...
    let input = {
        ...
        let is_child_viewport = ids.parent != ids.this;
