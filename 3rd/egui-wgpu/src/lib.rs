...
impl RenderState {
    ...
    pub async fn create(
        ...
        let target_format = if let Some(preferred) = config.preferred_surface_format {
            if surface_formats.contains(&preferred) {
                log::info!("Using preferred surface format: {preferred:?}");
                preferred
            } else {
                log::warn!(
                    "Preferred surface format {preferred:?} not supported; falling back to default"
                );
                crate::preferred_framebuffer_format(&surface_formats)?
            }
        } else {
            crate::preferred_framebuffer_format(&surface_formats)?
        };
...
pub struct WgpuConfiguration {
    ...
    /// Override the surface texture format.
    ///
    /// When set, the painter will prefer this format if the adapter supports it.
    /// Useful for HDR rendering with `Rgba16Float` on macOS EDR displays.
    ///
    /// `None` = use the default heuristic (`preferred_framebuffer_format`).
...
impl std::fmt::Debug for WgpuConfiguration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("WgpuConfiguration")
            ...
            .field("preferred_surface_format", &preferred_surface_format)
