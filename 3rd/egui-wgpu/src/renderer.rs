...
pub struct Renderer {
    ...
    /// The surface / render-target format that was passed to [`Self::new`].
    ///
    /// Used to choose the correct internal texture format for egui-managed
    /// textures (sRGB vs non-sRGB) so that hardware sRGB decode is applied
    /// when the shader expects linear inputs.
        ...
        let (texture, origin, bind_group) = if let Some(pos) = image_delta.pos {
            ...
            let texture = {
                ...
                device.create_texture(&wgpu::TextureDescriptor {
                    ...
                    format: egui_tex_format,
                    usage: wgpu::TextureUsages::TEXTURE_BINDING | wgpu::TextureUsages::COPY_DST,
                    view_formats: &[egui_tex_format],
                })
            };
