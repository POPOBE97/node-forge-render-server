...
impl ILightUpdateConfig {
    pub fn pack_buffer(&self, scene: &crate::dsl::SceneDSL) -> Vec<u8> {
        let nodes_by_id: std::collections::HashMap<&str, &crate::dsl::Node> = scene
            .nodes
            .iter()
            .map(|n| (n.id.as_str(), n))
            .collect();
...
pub(crate) fn assemble_intelligent_light(
    ...
    let driver_node_id = incoming_connection(scene, layer_id, "driver")
    ...
    let ilight_buffer_name: ResourceName =
        format!("params.sys.ilight.{layer_id}.graph").into();
    ...
    let upsample_pass_name: ResourceName =
        format!("sys.ilight.{layer_id}.upsample.pass").into();
...
const CHIP_RANDOM_OFFSETS: [f64; 33] = [
    -80.79679107666015,  -151.77667236328125,  289.5301513671875,
     133.59625244140625,   152.01351928710938,   90.911148071289070,
    -256.3884582519531,     78.98082733154297,  230.8242950439453,
    -136.3740234375,       -38.15315246582031,  159.8968505859375,
     -13.360940933227539, -157.33534240722655, -135.05589294433593,
     -84.4410171508789,   -200.09568786621093,   -8.089578628540039,
     238.59375,            245.52487182617186, -263.66140747070315,
     242.79183959960937,     2.713751792907715,    9.775185585021973,
    -108.58023834228516,   291.9852600097656,    -3.613990545272827,
    -140.31329345703125,  -245.5602569580078,   268.6585693359375,
    -255.75054931640624,     0.4242539405822754, -69.51470184326172,
];
...
fn compute_light_positions(chip_rotation: f64) -> [(f64, f64); 11] {
    ...
    for index in 0..11 {
        ...
        let noise1 =
            noise2_final(oi - 8.5, CHIP_RANDOM_OFFSETS[offset_index] + rotation_factor);
