    ...
    fn plan_prepared(
        ...
        let pass_bindings: Vec<PassBindings> = render_pass_specs
            ...
            .map(|spec| PassBindings {
                ...
                shader_parameter_binding: shader_parameter_buffers_by_pass
                    .get(&spec.pass_id)
                    .map(|plan| plan.binding.clone()),
                last_shader_parameter_hash: shader_parameter_buffers_by_pass
                    .get(&spec.pass_id)
                    .map(|plan| hash_bytes(plan.values.as_slice())),
...
mod tests {
    ...
    fn intelligent_light_node_id(plan: &RenderPlan) -> String {
        plan.prepared
            ...
            .iter()
            .find(|node| {
                node.node_type == "IntelligentLight"
                    && plan
                        .resources
                        .pass_output_registry
                        .get_for_port(&node.id, "pass")
                        .is_some()
            })
            .expect("planned scene should contain a registered IntelligentLight.pass output")
            .id
            .clone()
    }
    ...
    fn intelligent_light_composite_uses_intrinsic_output_and_edge_compose() -> Result<()> {
        ...
        assert_eq!(
            output.texture_name.as_str(),
            format!("sys.ilight.{node_id}.inter")
        );
        assert!(
            plan.resources
                .render_pass_specs
                .iter()
                .any(|spec| spec.name.as_str().contains(".compose.pass"))
        );
        assert!(
            plan.resources
                .render_pass_specs
        ...
        for (level, expected) in [
            ...
            let pass = add_passes
                ...
                .find(|spec| {
                    spec.name
                        .as_str()
                        .contains(&format!(".lvl{level}.add.pass"))
                })
                .expect("doubao Bloom add pass for authored level");
            assert_eq!(pass.params.color[..2], expected);
        }
    ...
    fn doubao_pass_resource_pool_isolates_single_and_matrix_variants() -> Result<()> {
        ...
        let pool_node = bloom_plan
            .prepared
            .matrix_source_scene
            .nodes
            .iter()
            .find(|node| {
                node.node_type == "ResourcePool" && node.id.ends_with("/ResourcePool_ca27a95e_57")
            })
            .expect("expanded Glow Render Scheme pool");
        assert_eq!(
            pool_node
                .inputs
                .iter()
                .map(|input| input.name.as_deref())
                .collect::<Vec<_>>(),
            vec![Some("Bloom Based"), Some("SDF Based")]
        );
        let pool_id = pool_node.id.clone();
        let bloom_pass_id = bloom_plan
            .prepared
            .matrix_source_scene
            .nodes
            .iter()
            .find(|node| node.id.ends_with("/RenderPass_ca27a95e_56"))
            .expect("expanded Bloom pass")
            .id
            .clone();
        let sdf_pass_id = bloom_plan
            .prepared
            .matrix_source_scene
            .nodes
            .iter()
            .find(|node| node.id.ends_with("/RenderPass_26"))
            .expect("expanded SDF pass")
            .id
            .clone();

        assert_pass_pool_variant_isolated(&bloom_plan, &pool_id, &bloom_pass_id, &sdf_pass_id);
        assert!(
            bloom_plan
                .prepared
                .matrix_source_scene
                .nodes
                .iter()
                .any(|node| node.id == sdf_pass_id)
        );

        let mut sdf_source = bloom_plan.prepared.matrix_source_scene.clone();
        sdf_source
            .nodes
            .iter_mut()
            .find(|node| node.id == pool_id)
            .expect("expanded Glow Render Scheme pool")
            .params
            .insert("selectedIndex".to_string(), serde_json::json!(1));
        let sdf_plan = planner_for_mode(ShaderSpacePresentationMode::SceneLinear).plan(
            &sdf_source,
            assets.as_ref(),
            None,
        )?;
        assert_pass_pool_variant_isolated(&sdf_plan, &pool_id, &sdf_pass_id, &bloom_pass_id);

        assert_ne!(
            crate::renderer::graph_uniforms::compute_pipeline_signature_for_pass_bindings(
                &bloom_plan.prepared.scene,
                &bloom_plan.resources.pass_bindings,
            ),
            crate::renderer::graph_uniforms::compute_pipeline_signature_for_pass_bindings(
                &sdf_plan.prepared.scene,
                &bloom_plan.resources.pass_bindings,
            ),
            "changing the selected pass must force a Single ShaderSpace rebuild"
        );
        Ok(())
    }

    fn assert_pass_pool_variant_isolated(
        plan: &RenderPlan,
        pool_id: &str,
        selected_pass_id: &str,
        inactive_pass_id: &str,
    ) {
        assert!(
            plan.prepared
                .scene
                .nodes
                .iter()
                .any(|node| node.id == selected_pass_id)
        );
        assert!(
            plan.prepared
                .scene
                .nodes
                .iter()
                .all(|node| node.id != inactive_pass_id && node.id != pool_id)
        );
        assert!(
            plan.resources
                .render_pass_specs
                .iter()
                .all(|spec| spec.pass_id != inactive_pass_id)
        );
        let selected_output = plan
            .resources
            .pass_output_registry
            .get_for_port(selected_pass_id, "pass")
            .expect("selected pass output must be planned");
        assert!(
            plan.resources
                .render_pass_specs
                .iter()
                .any(|spec| spec.target_texture == selected_output.texture_name)
        );
        assert!(
            plan.resources
                .pass_output_registry
                .get_for_port(inactive_pass_id, "pass")
                .is_none()
        );
        assert!(
            !format!("{:#?}", plan.resources).contains(inactive_pass_id),
            "inactive pass leaked into textures, buffers, shaders, or composition specs"
        );
        assert!(
            plan.pass_debug_sources
                .values()
                .all(|source| !source.module_source.contains(inactive_pass_id))
        );
    }
