//! HyperOS Glass material node.
//!
//! Cloned from GlassMaterial — same mechanism with a separate template for independent algorithm iteration.
...
fn resolve_pass_binding(
    ...
    let upstream = nodes_by_id.get(&upstream_id).ok_or_else(|| {
        anyhow::anyhow!(
            "HyperOSGlassMaterial: upstream node not found for {port_id}: {upstream_id}"
        )
    })?;

    if !is_pass_like_node_type(&upstream.node_type) {
        bail!(
            "HyperOSGlassMaterial.{port_id} must be connected to a pass node, got {}",
            upstream.node_type
        );
    }
...
const HYPEROS_GLASS_TEMPLATE_SPLIT: &str = "\n// {{BODY}}\n";

fn hyperos_glass_override_path(node: &Node) -> Option<std::path::PathBuf> {
    node.wgsl_override
        .as_deref()
        .and_then(super::template_loader::resolve_override_path)
}

fn hyperos_glass_template_parts(node: &Node) -> (String, String) {
    let path = hyperos_glass_override_path(node);
    let full = super::template_loader::load_template_with_override(
        path.as_deref(),
        "hyperos_glass_material_fragment.wgsl",
    );
    let split_pos = full
        .find(HYPEROS_GLASS_TEMPLATE_SPLIT)
        .expect("template missing // {{BODY}} marker");
    let helpers_raw = &full[..split_pos];
    let body = full[split_pos + HYPEROS_GLASS_TEMPLATE_SPLIT.len()..].to_owned();
...
fn hyperos_glass_lib_key_for(node: &Node) -> String {
    if node.wgsl_override.is_some() {
        let suffix: String = node
            ...
            .collect();
        format!("{HYPEROS_GLASS_WGSL_LIB_KEY}::{suffix}")
    } else {
        HYPEROS_GLASS_WGSL_LIB_KEY.to_string()
    }
}

fn ensure_hyperos_glass_wgsl_lib(ctx: &mut MaterialCompileContext, node: &Node) {
    super::sdf_nodes::ensure_default_sdf2d_wgsl_lib(ctx);

    let lib_key = hyperos_glass_lib_key_for(node);
    if ctx.extra_wgsl_decls.contains_key(&lib_key) {
        return;
    }

    let (helpers, _) = hyperos_glass_template_parts(node);
    let header = if node.wgsl_override.is_some() {
        format!(
            "\n// ---- HyperOSGlassMaterial helpers (generated, override for {}) ----\n\n",
            node.id
        )
    } else {
...
pub fn compile_hyperos_glass_material<F>(
    scene: &SceneDSL,
    nodes_by_id: &HashMap<String, Node>,
    node: &Node,
...
{
    ensure_hyperos_glass_wgsl_lib(ctx, node);
    ...
    let u_blend_darker = input_expr("uBlendDarker", input_f32_expr(node, "uBlendDarker", 0.15))?;
    let u_blend_darker_range_x = input_expr(
        "uBlendDarkerRangeX",
        input_f32_expr(node, "uBlendDarkerRangeX", 0.5),
    )?;
    let u_blend_darker_range_y = input_expr(
        "uBlendDarkerRangeY",
        input_f32_expr(node, "uBlendDarkerRangeY", 0.8),
    )?;
    let u_debug_shape_expand = input_expr(
        "uDebugShapeExpand",
        input_f32_expr(node, "uDebugShapeExpand", 2.5),
    )?;
    ...
    let uses_time = [
        ...
        u_blend_darker_range_x.uses_time,
        u_blend_darker_range_y.uses_time,
        u_debug_shape_expand.uses_time,
    ...
    fn expect_ty(expr: &TypedExpr, expected: ValueType, name: &str) -> Result<()> {
        if expr.ty != expected {
            bail!(
                "HyperOSGlassMaterial.{name} expected {:?}, got {:?}",
                expected,
                expr.ty
            );
        }
    ...
    expect_ty(
        &u_blend_darker_range_x,
        ValueType::F32,
        "uBlendDarkerRangeX",
    )?;
    expect_ty(
        &u_blend_darker_range_y,
        ValueType::F32,
        "uBlendDarkerRangeY",
    )?;
    expect_ty(&u_debug_shape_expand, ValueType::F32, "uDebugShapeExpand")?;
    ...
    let bg_color_expr = if let Some((_, tex, samp)) = bg_color.clone().or(bg.clone()) {
        format!(
            "hyperos_glass_texture_map({tex}, {samp}, screen_uv, {add_foreground}, {opacity}, {blend_mode}, {fg_tex}, {fg_samp}, screen_uv)",
    ...
    let refraction_expr = if let Some((_, tex, samp)) = refract.or(bg.clone()) {
        format!(
            "hyperos_glass_texture_map({tex}, {samp}, refract_uv, {add_foreground}, {opacity}, {blend_mode}, {fg_tex}, {fg_samp}, screen_uv)",
            opacity = u_reflect_lighten_opacity.expr,
            blend_mode = u_reflect_lighten_blend_mode.expr,
            fg_tex = fg_tex,
            fg_samp = fg_samp,
    ...
    let reflection_expr = if let Some((_, tex, samp)) = reflect.or(bg.clone()) {
        format!(
            "hyperos_glass_texture_map({tex}, {samp}, reflect_uv, {add_foreground}, {opacity}, {blend_mode}, {fg_tex}, {fg_samp}, screen_uv)",
            opacity = u_reflect_lighten_opacity.expr,
            blend_mode = u_reflect_lighten_blend_mode.expr,
            fg_tex = fg_tex,
            fg_samp = fg_samp,
    ...
    let mut vars: Vec<(&str, String)> = vec![
        ...
        ("uBlendBrightness", u_blend_brightness.expr.clone()),
        ("uBlendDarker", u_blend_darker.expr.clone()),
        ("uBlendDarkerRangeX", u_blend_darker_range_x.expr.clone()),
        ("uBlendDarkerRangeY", u_blend_darker_range_y.expr.clone()),
        ("uDebugShapeExpand", u_debug_shape_expand.expr.clone()),
    ...
    let (_, body_template) = hyperos_glass_template_parts(node);
