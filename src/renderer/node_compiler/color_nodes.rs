    ...
    F: Fn(&str, Option<&str>, &mut MaterialCompileContext, &mut HashMap<(String, String), TypedExpr>) -> Result<TypedExpr>,
{
    ...
    let fac = compile_fn(&fac_conn.from.node_id, Some(&fac_conn.from.port_id), ctx, cache)?;
    ...
    F: Fn(&str, Option<&str>, &mut MaterialCompileContext, &mut HashMap<(String, String), TypedExpr>) -> Result<TypedExpr>,
{
    ...
    let color = compile_fn(&color_conn.from.node_id, Some(&color_conn.from.port_id), ctx, cache)?;
